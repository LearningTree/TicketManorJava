...
Commands are from PostgreSQL but will be similar on other DB products.

----
website=# \c ticketmanor
You are now connected to database "ticketmanorjava"...

website=# create role myAppAcct login password 'someEnchantedPasswordHere45678';
CREATE ROLE

website=# revoke all on database ticketmanorjava from myAppAcct;
REVOKE

website=# grant select, insert, update on all tables in schema public to myAppAcct;
GRANT
website=# 
